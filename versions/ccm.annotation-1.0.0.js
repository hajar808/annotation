...
(() => {
    ...
    const component = {
        ...
        config: {
            "db": ["ccm.store", { name: 'ccm-annotation', url: "https://ccm2.inf.h-brs.de" }],
            "css": ["ccm.load",
  ...
  <p>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum
      ...
      <p>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum
        ...
        `,
            ...
            "js": ["ccm.load",
                [
                    ...
                    { "context": "head", "url": "https://hajar808.github.io/annotation/resources/js/analytics.js" }
                ]
            ],
            "title": "Annotation",
            "shadow": "none"
        ...
        Instance: function () {
            ...
            this.ready = async () => {
                /*
                      const data = await self.db.get();
                      data.forEach(entry => {
                          if (entry.key) {
                              self.db.del(entry.key)
                          }
                      })
                      */
            };
            ...
            this.start = async () => {
                ...
                if (text_content) {
                    text_content.innerHTML = self.content;
                }
                $.setContent(this.element, annotation);
                this.element.querySelector('#login_logout_area').appendChild(this.user.root);
                // After contetnt loaded inect sidenav and dropdown of materialize
                initDAO();
                injectSidenavAndDropDown();
                injectDB();
                injectUserKey();
                showAllHighlights();
            ...
            function injectSidenavAndDropDown() {
                let elems = document.querySelectorAll('.sidenav');
                M.Sidenav.init(elems, { edge: 'right' });
                jQuery("#group").dropdown();
                jQuery("#sort").dropdown();
            };

            function injectUserKey() {
                setUserKey(self.user.data().key);
            }
